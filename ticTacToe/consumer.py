...
def start(room_no):
    ...
    if game.players_entered == 1:
        ...
        game.room.reset()
        return {"action": "player_not_joined", "player": player}
    game.start()
    data = {"action": "started", "board": game.board, "state": game.get_new_state()}
    return data
...
class TACConsumer(AsyncConsumer):
    ...
    async def websocket_receive(self, event):
        ...
        await self.channel_layer.group_send(
            self.room_no, {"type": "send_message", "text": json.dumps(data)}
        )
    ...
    def check_run(self, username):
        ...
        if not game.zero_entered and not game.cross_entered:
            ...
            game.save()
            return True
        elif (game.zero_entered and not game.cross_entered) or (
            game.cross_entered and not game.zero_entered
        ):
            if game.zero_entered:
                game.cross_entered = True
            else:
    ...
    def play(self, msg, username):
        ...
        if game.current_player.username != username:
            return {"action": "error", "error_msg": "wait for your turn"}
        if game.board[i][j] != 0:
            return {"action": "error", "error_msg": "Invalid move"}
        if game.zero.username == username:
            game.board[i][j] = "Z"
        else:
            game.board[i][j] = "X"
