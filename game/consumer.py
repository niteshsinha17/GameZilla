...
class RoomConsumer(AsyncConsumer):
    ...
    def change_ready(self, username):
        ...
        room.save()
        data = {"action": "ready", "member": username, "state": member_.ready}
        return data
    ...
    def leave(self, username, is_host):
        if is_host:
            ...
            if room.started:
                return {
                    "action": "room_error",
                    "error_msg": "You can't leave right now",
                }
            room.delete()
            ...
            msg = {
                "action": "leaved",
                "member": username,
                "leaved_msg": username + " leaved the room",
    ...
    def remove_member(self, msg):
        ...
        room = Room.objects.get(sp_id=self.room_no)
        if room.started:
            return {"action": "room_error", "error_msg": "can't remove right now"}
        members = Member.objects.filter(room=room)
        member = members.get(member=user)
    ...
    def start_game(self):
        room = Room.objects.get(sp_id=self.room_no)
        if room.started:
            return {"action": "room_error", "error_msg": "Game has already started"}
