...
def game(request, game_id):
    ...
    try:
        ...
        messages.add_message(request, messages.INFO, "Game do not exists")
        return redirect("home")
    players = game.players.all()
    try:
        player = players.get(player=user)
    except:
        messages.add_message(request, messages.INFO, "You are not allowed")
        return redirect("home")
    ...
    if game.started and (player.leaved or not player.entered):
        messages.add_message(
            request,
            messages.INFO,
            "Cant join, You already leaved or removed due to network",
        )
        return redirect("home")
