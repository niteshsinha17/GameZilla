...
def start(room_no):
    ...
    if game.players_playing <= 1:
        ...
        game.delete()
        game.room.reset()
        return {"action": "player_not_joined", "players": players}
    game.start()
...
class SNLConsumer(AsyncConsumer):
    async def websocket_connect(self, event):
        ...
        if game_started:
            ...
            if not can_enter:
                ...
                return await self.send({"type": "websocket.disconnect"})
                ...
                await self.channel_layer.group_add(self.room_no, self.channel_name)
                ...
                await self.channel_layer.group_send(
                    self.room_no,
                    {
                        "type": "send_message",
                        "text": json.dumps(
                            {"action": "online", "player": username, "status": True}
                        ),
                    },
                )
    ...
    async def websocket_receive(self, event):
        ...
        await self.channel_layer.group_send(
            self.room_no, {"type": "send_message", "text": json.dumps(state)}
        )
    ...
    async def websocket_disconnect(self, event):
        ...
        await self.channel_layer.group_send(
            self.room_no,
            {
                "type": "send_message",
                "text": json.dumps(
                    {"action": "online", "player": username, "status": False}
                ),
            },
        )
    ...
    def leave(self, username):
        ...
        players = game.players.filter(entered=True, disable=False, leaved=False)
        if game.players_playing == 1:
            game.winner_state += 1
            winner = players[0]
    ...
    def check_state(self, state):
        ...
        if game.match_state(state):
            ...
            if player.skip_chance == 0:
                ...
                game.players_playing -= 1
                room = game.room
                member = Member.objects.filter(member=player.player, room=room)[0]
                member.leaved = True
                member.save()
                ...
                if game.players_playing == 1:
                    ...
                    game.winner_state += 1
                    winner = players.filter(entered=True, disable=False, leaved=False)[
                        0
                    ]
                    winner.rank = game.winner_state
                    winner.disable = True
            ...
            game.save()
            msg = {"action": "set_state", "error": True, "state": game.get_new_state()}
    ...
    def change_player(self):
        ...
        game.time_stamp = time.time()
        game.save()
        data = {"action": "player_changed", "state": game.get_new_state()}
